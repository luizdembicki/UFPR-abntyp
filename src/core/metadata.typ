...
/// - objetivo: objetivo
/// - texto-apresentacao: bloco livre da folha de rosto (override de natureza/objetivo)
/// - area: área de concentração
/// - orientador: orientador
/// - coorientador: coorientador (opcional)
/// - rotulo-orientador: rótulo do orientador (padrão: "Orientador")
/// - rotulo-coorientador: rótulo do coorientador (padrão: "Coorientador")
...
#let _str-safe(val) = {
  if type(val) == str { val } else if type(val) == int or type(val) == float { str(val) } else if val == none {
    ""
  } else if type(val) == content { val } else { str(val) }
}

/// Renderiza título e subtítulo no padrão ABNT.
/// Título em maiúsculas + ":" + subtítulo em tamanho diferenciado.
///
...
#let _render-titulo(
  ...
  if titulo != none {
    if subtitulo != none {
      text(size: tamanho-titulo, upper(titulo) + ":")
      linebreak()
